...
/**
 * Helps build 108jobs HTTP requests.
 */
@Route("api/v4")
export class CustomHttp extends Controller {
    #apiUrl: string;
    #headers: { [key: string]: string } = {};
    #fetchFunction: typeof fetch = fetch.bind(globalThis);

    /**
     * Generates a new instance of CustomHttp.
    ...
    /**
     * @summary Search 108jobs. If `search-term` is a url it also attempts to fetch it, just like `resolve-object`.
     */
    @Security("bearerAuth")
    @Security({})
    @Get("/search")
    ...
    async #wrapper<BodyType extends object, ResponseType>(
        ...
        if (!response.ok) {
            throw new CustomError(json.error ?? response.statusText, json.message);
        } else {
            return json;
        }
    }
