...
import {Person, PortfolioPic, SaveUserSettings} from '108jobs-client';
...
export interface PortfolioImagesFormProps {
    person?: Person;
...
export const usePortfolioImagesForm = ({
    ...
    const initialPortfolioImages: PortfolioPic[] = person?.portfolioPics ?? [];
    ...
    // Sync form with external initialPortfolioImages changes
    useEffect(() => {
        if (initialPortfolioImages.length > 0) {
            reset({
                portfolioImages: initialPortfolioImages,
            });
        }
    }, [initialPortfolioImages, reset]);
